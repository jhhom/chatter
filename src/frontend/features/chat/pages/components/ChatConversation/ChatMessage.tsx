...
import { LexicalRenderer } from "~/frontend/features/chat/pages/components/ChatConversation/lib/lexical";
      ...
      {match(props.content)
        ...
        .with({ type: "file" }, (m) => (
          <div className="flex h-16 items-center rounded-md px-1">
            <a
              className={clsx("flex h-14 items-center px-3", {
  ...
  return (
    <div className="group relative ml-auto mt-1 w-fit max-w-[calc(100%-10%)] rounded-md bg-gray-50 text-[0.8rem]">
      ...
      <div className="mt-0.5 flex justify-end px-4 pb-2">
        ...
        <div
          className={clsx("flex items-end justify-start pb-0.5", {
            "pl-1.5": props.read,
            "pl-0.5": !props.read,
          })}
        >
      ...
      <div id={`message-${props.seqId}`}>
        {props.seq === "first" ||
          (props.seq === "single" && (
            ...
            >
              <p
                className={clsx("rounded-md bg-white px-2 py-1 text-sm", {
