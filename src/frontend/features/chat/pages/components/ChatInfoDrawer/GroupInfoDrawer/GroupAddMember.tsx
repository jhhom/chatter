...
import { IconX } from "~/frontend/features/common/icons";
            ...
            <div className="flex flex-wrap items-start justify-start gap-x-1 gap-y-1">
              {addedMembers.map((m) => (
                <ContactPill
                  ...
                  onRemove={(userId) => {
                    setAddedMembers(
                      addedMembers.filter((x) => x.userId != userId)
                    );
                    setNewMemberSearchList(
                      newMemberSearchList.concat([
                        {
          ...
          {newMemberSearchList.map((m) => (
            <Contact
              ...
              onClick={() => {
                ...
                setNewMemberSearchList(
                  newMemberSearchList.filter((u) => m.userId !== u.userId)
                );
              }}
            />
          ))}
