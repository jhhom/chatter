...
// https://codesandbox.io/s/falling-sky-iu5g6u?file=/src/Form.tsx
...
export function SignIn(props: {
  login: IApiClient["auth/login"];
  onLoginSuccess: () => Promise<Result<unknown, unknown>>;
}) {
  const {
    register,
    handleSubmit,
  ...
  const onLoginSuccess = useMutation({
    mutationFn: props.onLoginSuccess,
    onSuccess: (data) => {
      if (data.isErr()) {
  ...
  const login = useMutation({
    mutationFn: props.login,
    onSuccess: (data) => {
      if (data.isErr()) {
        setOpenErrorAlert(true);
      }
      void props.onLoginSuccess();
    },
    onError: (error) => {
      setOpenErrorAlert(true);
    },
  ...
  const hasLoginError =
    (login?.data !== undefined && login.data.isErr()) ||
    login.isError ||
    (onLoginSuccess?.data !== undefined && onLoginSuccess.data.isErr()) ||
    onLoginSuccess.isError;
  ...
  const loginErrorMessage =
    ...
    (onLoginSuccess.data !== undefined && onLoginSuccess.data.isErr())
      ...
      : login.data !== undefined && login.data.isErr()
      ? match(login.data.error.details.type)
          .with("AUTH.INCORRECT_PASSWORD", () => "Password is incorrect")
          .with("RESOURCE_NOT_FOUND", () => "Username is incorrect")
          .otherwise(() => "An unexpected error has occured")
      : "No error";
  ...
  return (
    <div className="flex h-screen min-h-full">
      <div className="flex basis-full flex-col justify-center py-12">
        <div className="mx-auto w-4/5 max-w-sm">
          <div>
            <div className="flex">
          ...
          <div
            className={clsx("flex items-end", {
              "h-20": hasLoginError && openErrorAlert,
              "h-10": !(hasLoginError && openErrorAlert),
            })}
          >
            {openErrorAlert && hasLoginError && (
                ...
                <div>
                  <button
                    ...
                    className={clsx(
                      ...
                      { "hover:bg-primary-600": !login.isLoading },
                      { "cursor-not-allowed bg-primary-300": login.isLoading }
                    )}
                  >
                    {login.isLoading && (
