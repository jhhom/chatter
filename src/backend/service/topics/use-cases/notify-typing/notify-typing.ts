...
import { GroupTopicId, TopicId, UserId } from "~/backend/drizzle/schema";
...
export default async function notifyIsTyping(
  ...
  if (IsUserId(arg.topicId)) {
    ...
    const subs = await getSubscribersOfTopic(ctx.db, { topicId: arg.topicId });
    if (subs.isErr()) {
