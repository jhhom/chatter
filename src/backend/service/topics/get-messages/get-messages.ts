...
import { getP2PTopicMessages } from "~/backend/service/topics/get-messages/get-p2p-topic-messages";
import { getGroupTopicMessages } from "~/backend/service/topics/get-messages/get-group-topic-messages";
import {
  UserId,
  GroupTopicId,
  TopicId,
...
export async function getMessages(
  ...
  if (IsGroupTopicId(topicId)) {
    ...
    const userStatus = userStatusResult.value;
    if (userStatus.type == "user is never in the group") {
      return err(
        new AppError("UNKNOWN", { cause: "user is never in the group" })
      );
    }
